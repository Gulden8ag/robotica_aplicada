...
For an $n$-DOF manipulator, the Jacobian is constructed as:
$$
J = \begin{bmatrix} J_1 & J_2 & \dots & J_n \end{bmatrix}
$$
For the $i$-th joint, the column $J_i$ is a $6 \times 1$ vector composed of a linear velocity part ($J_{v_i}$) and a rotational (angular) velocity part ($J_{\omega_i}$):
$$
J_i = \begin{bmatrix} J_{v_i} \\ J_{\omega_i} \end{bmatrix}
$$
Depending on whether joint $i$ is prismatic (sliding) or revolute (rotating), we construct its column using the formulas in the following table:
$$\begin{array}{c|c|c}
